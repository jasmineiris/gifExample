...
- (IBAction)createGif {
    ...
    int x = 1;
    UIImage *imagezor = [[UIImage alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:[NSString stringWithFormat:@"%d", x] ofType:@"jpg"]];
    NSLog(@"%@", imagezor);
    while(imagezor != NULL) {
        [frames addObject:imagezor];
        x++;
        imagezor = [[UIImage alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:[NSString stringWithFormat:@"%d", x] ofType:@"jpg"]];
        NSLog(@"%@", imagezor);
    }
    ...
    NSLog(@"Going to write to file, go TEAM JAZ");
    
    [data writeToFile:path atomically:YES];
    NSLog(@"Wrote to file, YAY codepath will have an app");
